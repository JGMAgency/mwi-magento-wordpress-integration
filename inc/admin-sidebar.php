<?php global $jck_mwi; ?>

<div id="mwi_right">
	<div class="mwi_meta">
	    
	    <h3><?php _e('Add-ons', $jck_mwi->slug); ?></h3>
		<ul>
		    <li>
		        <strong>Shortcodes</strong> &mdash; <a href="http://bit.ly/1GKrOiP" target="_blank">Buy Now for $18</a><br>
		        Use the shortcode button to <em>insert products</em>, <em>layout blocks</em>, and <em>static blocks</em> in your WordPress editor.
            </li>
            <li>
		        <strong>Category Listing</strong> &mdash; <a href="http://bit.ly/1GKrOiP" target="_blank">Buy Now for $18</a><br>
		        Use the shortcode button to insert a <em>paginated category listing</em> of your Magento products in your WordPress editor.
            </li>
		</ul>
	
		<h3><?php _e('Documentation', $jck_mwi->slug); ?></h3>
		<p>Visit <a href="http://www.mwi-plugin.com" target="_blank">www.mwi-plugin.com</a> for all the latest documentation; including usage instructions, functions, examples and more.</p>
		
		<h3><?php _e('Support', $jck_mwi->slug); ?></h3>
		<p>If you require some support, please check out the <a href="http://mwi-plugin.com/documentation/faq/" target="_blank">FAQ section</a> on the MWI website. If that doesn't help, try using the <a href="madebyplatform.com/support-request-form/" target="_blank">support form</a>. If all else fails, please <a href="mailto:support@madebyplatform.com" target="_blank">send an email</a>.</p>
		
		<h3><?php _e('Twitter', $jck_mwi->slug); ?></h3>
		<a href="https://twitter.com/jamesckemp" class="twitter-follow-button" data-show-count="false">Follow @jamesckemp</a>
<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>
		
		<h3><?php _e('Credits', $jck_mwi->slug); ?></h3>	
		<p>Created by <a href="http://www.jckemp.com" target="_blank">James Kemp</a></p>
			
	</div>
</div><!-- /#mwi_right -->